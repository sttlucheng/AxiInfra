...
- AR重排表更新逻辑
    ...
    - nid更新：
        ...
        - 当 slave R 通道返回某个读事务的最后一个数据且握手成功时，表示该读事务已经完成。模块使用 R 返回 ID 定位对应的 AR 重排表表项，并读取其中保存的原始 master AR ID。随后模块遍历所有有效的 AR 重排表表项，对保存了相同原始 ID 且 nid 非 0 的表项进行 nid 更新。

            - 通常情况下，每完成一个同 ID 的前序读事务，后续同 ID 表项的 nid 执行 nid - 1。这里的 nid 表示该表项前方尚未完成的同 ID 读事务数量，因此前序事务完成后，后续事务的等待计数需要减少 1。

            - 此外还需要处理一个同周期边界：当 slave R 通道 last beat 握手成功的同一周期，master AR 通道也握手接收了一笔相同 ID 的新读事务时，新事务在写入 AR 重排表时计算得到的 nid 仍可能包含这笔“本周期即将完成”的前序事务。为修正该情况，模块使用 rWkVld/rWkVldReg/rWkEtrReg 记录这个同周期事件，并在下一拍对新分配的表项额外执行一次 nid - 1。

            - 如果某个表项在同一拍既命中普通的 R last 完成更新，又命中 rWkVldReg 指示的同周期修正，则说明它需要同时扣除两个已完成的同 ID 前序事务，因此该表项执行 nid - 2。通过 nid - 1 和 nid - 2 两类更新，模块保证后续同 ID 读事务只有在所有前序同 ID 事务完成后，才会变为 nid == 0，并被允许向 slave 发送 AR 请求。
...
- AW重排表更新逻辑
    ...
    - nid更新：
        ...
        -  当 slave B 通道返回某个写事务的响应且握手成功时，表示该写事务已经完成。模块使用 B 返回 ID 定位对应的 AW 重排表表项，并读取其中保存的原始 master AW ID。随后模块遍历所有有效的 AW 重排表表项，对保存了相同原始 ID 且 nid 非 0 的表项进行 nid 更新。

            - 通常情况下，每完成一个同 ID 的前序写事务，后续同 ID 表项的 nid 执行 nid - 1。

            - 此外还需要处理一个同周期边界：当 slave B 通道握手成功的同一周期，master AW 通道也握手接收了一笔相同 ID 的新写事务时，新事务在写入 AW 重排表时计算得到的 nid 仍可能包含这笔“本周期即将完成”的前序写事务。为修正该情况，模块使用 wWkVld/wWkVldReg/wWkEtrReg 记录这个同周期事件，并在下一拍对新分配的表项额外执行一次 nid - 1。

            - 如果某个表项在同一拍既命中普通的 B response 完成更新，又命中 wWkVldReg 指示的同周期修正，则说明它需要同时扣除两个已完成的同 ID 前序写事务，因此该表项执行 nid - 2。通过 nid - 1 和 nid - 2 两类更新，模块保证后续同 ID 写事务只有在所有前序同 ID 写事务完成后，才会变为 nid == 0，并被允许向 slave 发送 AW 请求。
...
- 第一个是代码疑点：
    ...
    val bFireSlvHit = io.slv.b.fire && arinfo(slvBHitEtr).bits.id === awinfo(i).id && wvld(i)
